...
class MicroServerSettingTab extends obsidian.PluginSettingTab {
  ...
  displayRemoteTab(container) {
    ...
    if (!isPro) {
      ...
      upgradeBanner.innerHTML = `
        ...
        <div style="color: var(--text-muted); margin-bottom: 15px;">
          ...
          Starts at <strong>$1.99/month</strong> with Base plan.
        </div>
    ...
    // Show upgrade CTA if not Pro (Glass Wall - still show controls below)
    if (!hasGuestAccess) {
      const upgradeCTA = guestManagerDiv.createDiv();
      upgradeCTA.style.cssText = 'padding: 20px; margin-bottom: 20px; text-align: center; background: rgba(255, 215, 0, 0.1); border: 2px solid #ffd700; border-radius: 6px; color: var(--text-normal);';
      upgradeCTA.innerHTML = `
        <div style="font-size: 32px; margin-bottom: 10px;">🔒</div>
        <div style="font-weight: bold; font-size: 1.1em; margin-bottom: 8px;">Pro Feature: Guest Vault Sharing</div>
        <div style="color: var(--text-muted); margin-bottom: 15px;">
          Share your vault with guests using read-only or read-write access.<br>
          Available on <strong>Pro plan ($3.99/month)</strong>.
        </div>
        <button class="mod-cta" style="padding: 10px 30px; background: #ffd700; color: #000; border: none; border-radius: 4px; cursor: pointer; font-weight: bold;">
          Upgrade to Pro
        </button>
    ...
    const emailInput = addGuestForm.createEl('input', { 
      type: 'email', 
      placeholder: hasGuestAccess ? 'guest@example.com' : 'Pro required' 
    });
    ...
    emailInput.disabled = !hasGuestAccess;
    
    // Label input
    const labelInput = addGuestForm.createEl('input', { 
      type: 'text', 
      placeholder: hasGuestAccess ? 'Display name (e.g., "Bob")' : 'Pro required' 
    });
    ...
    labelInput.disabled = !hasGuestAccess;
    
    // Password input
    const guestPassInput = addGuestForm.createEl('input', { 
      type: 'password', 
      placeholder: hasGuestAccess ? 'Password for guest' : 'Pro required' 
    });
    ...
    guestPassInput.disabled = !hasGuestAccess;
    ...
    modeSelect.disabled = !hasGuestAccess;
    
    // Add button
    const addGuestBtn = addGuestForm.createEl('button', { text: hasGuestAccess ? 'Add Guest' : 'Add Guest (Pro Required)' });
    ...
    addGuestBtn.disabled = !hasGuestAccess;
    ...
    // Status & Info Section (only show when active)
